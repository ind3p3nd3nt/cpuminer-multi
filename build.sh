...
if [ -f /usr/bin/apt ]; then apt update && apt install build-essential zlib1g-dev libssl-dev libcurl4-openssl-dev -y; else yum groupinstall 'Development Tools' -y && yum install libcurl-devel openssl-devel zlib -y; fi
